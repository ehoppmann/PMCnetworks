...
c.execute('''CREATE TABLE cites (pmid integer, incites text, outcites text, PRIMARY KEY (pmid))''')
i=0
for node in g.vertices():
    ...
    if i % 500000 == 0: print i #to monitor status
    c.execute('''COMMIT''')
c.close()
...
c.execute('''CREATE TABLE coauthors (author text, coauthors text, PRIMARY KEY (author))''')
i=0
for node in g.vertices():
    coauthors = ""
    coauthcount = 0
